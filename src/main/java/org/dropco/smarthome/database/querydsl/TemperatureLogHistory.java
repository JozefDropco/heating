...
public class TemperatureLogHistory extends com.querydsl.sql.RelationalPathBase<TemperatureLogHistory> {
    ...
    public void addMetadata() {
        ...
        addMetadata(average, ColumnMetadata.named("TEMP_VALUE").withIndex(2).ofType(Types.NUMERIC).withSize(50).withDigits(5).notNull());
        addMetadata(minimum, ColumnMetadata.named("MIN_VALUE").withIndex(3).ofType(Types.NUMERIC).withSize(50).withDigits(5).notNull());
        addMetadata(maximum, ColumnMetadata.named("MAX_VALUE").withIndex(4).ofType(Types.NUMERIC).withSize(50).withDigits(5).notNull());
        addMetadata(asOfDate, ColumnMetadata.named("AS_OF_DATE").withIndex(5).ofType(Types.TIMESTAMP).withSize(11).withDigits(6).notNull());
    }
